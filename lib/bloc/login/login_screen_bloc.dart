...
class LoginScreenBloc extends Bloc<LoginScreenEvents, LoginScreenStates, void> {
  ...
  bool _validLogin() {
    if (_login.isEmpty ||
        _login.length < MIN_LENGTH ||
        _login.length > MAX_LENGTH_LOGIN) {
      return false;
    }
    int min = MIN_LENGTH - 1;
    int max = MAX_LENGTH_LOGIN - 1;
    bool validLogin =
        RegExp('^[a-zA-Z][a-zA-Z0-9]{$min,$max}\$').hasMatch(_login);
    bool validEmail =
        RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(_login);
  ...
  bool _validUserName() {
    ...
    bool validUserName =
        RegExp('^[a-zA-Z][a-zA-Z 0-9]{$min,$max}\$').hasMatch(_userName);
  ...
  void _loginQB() async {
    ...
    try {
      QBLoginResult qbLoginResult =
          await _authRepository.login(_login, DEFAULT_USER_PASSWORD);
  ...
  void _createUser() async {
    try {
      QBUser? user = await _usersRepository.createUser(
          _login, _userName, DEFAULT_USER_PASSWORD);
      if (user != null) {
        _loginQB();
      } else {
