...
class ChatScreenBloc
  ...
  @override
  void setArgs(ChatArguments args) {
    _dialogId = args.dialogId;
    _isNewChat = args.isNewChat;
  }
  ...
  Future<void> onReceiveEvent(ChatScreenEvents receivedEvent) async {
    ...
    if (receivedEvent is SendMessageEvent) {
      if (receivedEvent.textMessage == null) {
        states?.add(
          SendMessageErrorState("Message is empty", null),
        );
        return;
      }
      ...
      try {
        ...
        states
            ?.add(SendMessageErrorState(e.message, receivedEvent.textMessage));
      }
    }
    ...
    if (receivedEvent is UsersAddedEvent) {
      // if (receivedEvent.addedUsersIds
      //     .contains(ChatInfoScreen.NO_USERS_SELECTED)) {
      //   return;
      // }
      try {
        _chatRepository
          ...
          _dialog = dialog;
          _usersRepository
              .getUsersByIds(receivedEvent.addedUsersIds)
              .then((addedUsers) {
            _sendNotificationMessageAddedUsers(addedUsers);
            _sendSystemMessagesAddedUsers(
                _dialog?.occupantsIds); // to occupants
          });
        });
      } on RepositoryException catch (e) {
        states?.add(ErrorState(e.message));
    ...
    if (receivedEvent is LeaveChatEvent) {
      ConnectionType connectionType =
          await _deviceRepository.checkInternetConnection();
      if (connectionType == ConnectionType.none) {
        states?.add(LeaveChatErrorState(
            "Unable to perform operation without connection"));
        return;
      }
  ...
  void _sendSystemMessagesAddedUsers(List<int>? ids) {
    for (int id in ids!) {
      _chatRepository.sendSystemMessage(
          _dialogId, id, ChatRepository.NOTIFICATION_TYPE_ADD);
    }
  }
  ...
  Future<void> _subscribeDeliveredStatus() async {
    ...
    try {
      _messageDeliveredSubscription = await QB.chat.subscribeChatEvent(
  ...
  Future<void> _subscribeConnectedChat() async {
    ...
    try {
      _connectedChatSubscription = await QB.chat.subscribeChatEvent(
          QBChatEvents.CONNECTED, _processConnectedChatEvent);
    } on PlatformException catch (e) {
      states?.add(ErrorState(makeErrorMessage(e)));
    }
  }
